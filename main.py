...
def add_question():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT max(id)+1 FROM sb_question")
        maxid = mycursor.fetchone()[0]
        if maxid is None:
            maxid=1
        sql = "INSERT INTO sb_question(id, dept, semester, subject_id, mark, kl, co, question) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        val = (maxid, dept, sem, sid, mark, '', '', question)
        act="success"
        mycursor.execute(sql, val)
        mydb.commit()            
        print(mycursor.rowcount, "record inserted.")
    ...
    return render_template('add_question.html', msg=msg, data=data, value=value, dept=dept, sem=sem, sid=sid)
...
def add_image():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("update sb_question set q_image=%s where id=%s",(fn,qid))
        mydb.commit()
        msg="success"

    return render_template('add_image.html',msg=msg,sid=sid)
...
def upload():
    ...
    if request.method=='POST':
        ...
        for ds in dat.values:
            ...
            if maxid is None:
                maxid=1
            sql = "INSERT INTO sb_question(id, dept, semester, subject_id, mark, kl, co, question) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            val = (maxid, dept1, sem1, sid1, mark, kl, co, question)
            act="success"
            mycursor.execute(sql, val)
            mydb.commit()            
    ...
    return render_template('upload.html', msg=msg, data=data, value=value, dept=dept, sem=sem, sid=sid)
...
def view_question():
    ...
    if request.method=='POST':
        ...
        if mark=="":
            ...
            mycursor.execute("SELECT * FROM sb_question where subject_id=%s && mark=%s",(sid,mark))
            data2 = mycursor.fetchall()
...
def view_paper():
    ...
    for ds2 in dd2:
        ...
        for v1 in val1:
            ...
            mycursor.execute("SELECT * FROM sb_question where id=%s",(v1[5],))
            ...
            if v1[10]==2:
                ...
                for v2 in val2:
                    ...
                    #print(v2[5])
                    #dt3.append("1")
                    ##mycursor.execute("SELECT * FROM sb_question where id=%s",(v2[5],))
...
def savepdf():
    ...
    rendered_html = render_template('view_paper.html',pid=3)
...
@app.route('/test')
def test():
    return render_template('test.html', title='Flask PDF Example')

@app.route('/generate_pdf')
def generate_pdf():
    rendered = render_template('test.html', title='Flask PDF Example')
